<?php
    ...
    class NotesModel extends \Core\Model {
        ...
        public function get_count($uid){
            self::init_config();
            $this->sql = $this->lib->count_sql(array('table'=>'cloudnote_notes','wh'=>array('user_id'=>$uid,'isactive'=>1)));
        ...
        public function output($uid,$pageno){
            ...
            $this->sql = $this->lib->select_sql(array('table'=>'cloudnote_notes','wh'=>array('user_id'=>$uid,'isactive'=>1),
            'index'=>$this->index,'range'=>$GLOBALS['config']['pagesize'],'order'=>'desc'));
